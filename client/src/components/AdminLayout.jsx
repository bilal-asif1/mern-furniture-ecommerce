  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="grid lg:grid-cols-[220px_1fr]">
        <aside className={`fixed inset-y-0 left-0 z-40 w-[220px] transform border-r border-black/5 bg-[#efe7df] transition-transform duration-300 lg:static lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
          <div className="flex h-full flex-col">
            ...
            {items.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `mb-2 block rounded-2xl px-3 py-2.5 text-sm font-semibold transition ${isActive ? 'bg-primary text-white shadow-soft' : 'text-text/70 hover:bg-white/60'}`
                }
              >
                {item.label}
              </NavLink>
